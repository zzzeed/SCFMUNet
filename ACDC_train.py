...
parser.add_argument("--batch_size", default=4, help="batch size")  # 12
...
for epoch in tqdm(range(args.max_epochs)):
    ...
    net.load_state_dict(torch.load(save_model_path1))
    test_save_path = None
    db_test = ACDCdataset(base_dir=args.volume_path, list_dir=args.list_dir, split="test")
    testloader = DataLoader(db_test, batch_size=1, shuffle=False)

    performance, mean_hd95, mean_jacard, mean_asd = inference(args, net, testloader, test_save_path)
    if performance > Best_performance:
        save_model_path = os.path.join(snapshot_path, 'best_' + str(epoch) + '.pth')
        torch.save(net.state_dict(), save_model_path)
        logging.info("save model to {}".format(save_model_path))
        Best_performance = performance

    if epoch >= args.max_epochs - 1:
        save_model_path = os.path.join(snapshot_path, 'last_epoch.pth')
        torch.save(net.state_dict(), save_model_path)
